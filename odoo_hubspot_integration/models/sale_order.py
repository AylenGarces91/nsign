...
class SaleOrder_Invoice_HubSpot(models.TransientModel):
    
    _inherit = "sale.advance.payment.inv"


    def create_invoices(self):
        toreturn = super(SaleOrder_Invoice_HubSpot, self).create_invoices()
        
        sale = self.env['sale.order'].browse(self._context.get('active_ids', []))
        account_move = self.env['account.move'].browse(toreturn.get('res_id'))
        
        if sale.amount_total == account_move.amount_total:
